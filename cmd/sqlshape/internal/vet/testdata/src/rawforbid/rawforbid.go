...
func f(ctx context.Context, c *pgx.Conn, id int64) {
	c.Query(ctx, byID, id)                                        // want `pgx.Query executes SQL outside sqlshape; with -raw-sql=forbid every statement goes through sqlshape.Query / One / postgres.Copy \(or list the package in -raw-sql-allow\)`
