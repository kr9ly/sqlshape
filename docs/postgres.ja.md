...
| `T[]` | `[]Go(T)`（各要素は単体の`T`のパラメータ/列と同じ判定を受ける。注記も含めて）。PostgreSQLは配列の要素自体がNOT NULLであることを保証しない――列自身の`NOT NULL`は配列値全体が`NULL`になることを禁じるだけである――ため、`NULL`を受けられない要素型（ポインタでないもの）には常時の注記が付き、`-strict`では拒否としても報告される。`integer[]`に対する`[]int32`、複合型の配列に対する`[]Item`はどちらも`NULL`要素を安全に受けるために`[]*int32`/`[]*Item`が要る（複合型の要素はより厄介で、pgxは`NULL`要素をエラーにせずゼロ値の構造体として黙って復号する）。要素が決して`NULL`にならないと検査器が証明できるときは注記も`-strict`の拒否も付かない: それ自体が`NULL`にならない`x`の`array_agg(x)`（行コンストラクタ`(a, b)::T`、内部結合した`NOT NULL`列）、要素がすべて非NULLのリテラル`ARRAY[...]`、単一の`NOT NULL`出力列にかかる`ARRAY(SELECT ...)`、そしてそれらを定義に持つビューを通して。素の配列列は対象外である。PostgreSQLは列の要素についてそのような宣言を持たないので、自分の方が正しいと分かっているならフィールドの`col:",notnull"`が要素についても主張する |
