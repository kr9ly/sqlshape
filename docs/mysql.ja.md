...
引数の値で実行時失敗が決まる関数は、引数が定数のとき判定する。失敗の置き場所は上の定数算術と同じ規則（畳み込まれる項は文エラー、行ごとに走る項は自分のエラー番号をキーにした violation、0行のクエリは失敗しない）。

| 関数 | 失敗する条件 | エラー |
|---|---|---|
| `INET_ATON` | 値が「最大255の数字グループをドット3個以下で区切った形」でない（`'122.256'`、末尾ドット。`'1.2.3'`は通る） | 1411、`IGNORE`以外のstrict書き込みのみ |
| `INET6_ATON` | 完全なドット4区切りIPv4でも正しいIPv6テキスト（`::`ギャップは1つ、グループは16進4桁まで）でもない | 1411、同じ条件 |
| `UNHEX` | 16進以外の文字を含む | 1411、同じ条件 |
| `STR_TO_DATE` | フォーマットで値が読めない（`extract_date_time`の指定子、en_USの月名・曜日名、`%V`/`%v`週と対応する`%X`/`%x`年）、または日付がセッションのゼロ日付フラグに反する（`NO_ZERO_DATE`下では日付結果の年・月・日いずれかの0も） | 1411。読めた値の後ろに空白以外が残ると1292（型語はフォーマットが決める結果型） |
| `UUID_TO_BIN` | 値が「16進32桁」「8-4-4-4-12のダッシュ形」「ダッシュ形を波括弧で囲んだ形」のどれでもない | 1411、文種・モードを問わない |
| `BIN_TO_UUID` | 値がちょうど16バイトでない | 1411、文種・モードを問わない |
| `PERIOD_ADD` / `PERIOD_DIFF` | period引数が「月1〜12の正の`[YY]YYMM`」でない | 1210、文種・モードを問わない |

行ごとのviolationは`mysql.Violates(err, "1411")` / `"1210"`が照合する。もう一系統は解決時に拒まれる — 式がどこにあっても、行が有っても無くても: `NAME_CONST`は両引数がリテラル（単項マイナス1段と`COLLATE`は可、畳み込みの`1+1`や`TRUE`は不可。名前がNULLなら1382）、`LIKE`の`ESCAPE`は1文字以下の定数、`NTILE`は正の個数・`NTH_VALUE`は正の整数位置、`MATCH`は1つのリレーションの列（selectの別名や2表跨ぎは1210 `to MATCH`）で`AGAINST`は定数（列を含むと1210 `to AGAINST`）。

### 関数呼び出しの引数とシステム変数の読み取り

関数呼び出しの引数に別名が付いている形（`f(x AS a)`、ローダブル関数の構文）は、引数を解決するより前にサーバと同じ順で拒む。ネイティブ関数はまず引数の個数（1582、名前は書かれたまま）、次に別名が1583（名前は小文字化される。`SELECT ABS(3 AS three)`は`... native function 'abs'`）。それ以外の名前 — ストアド関数、存在しない名前も同じ — は関数を探すより前に1584になる。データディクショナリの内部関数（`INTERNAL_TABLE_ROWS`などサーバが内部と印を付けるもの）は、どちらの検査よりも前に、文が名前を書いた時点で3566になる。

スコープを明示したシステム変数の読み取りは、変数自身のスコープと一致しなければならない。GLOBAL専用変数の`@@session.x`（`@@local.x`も同じ）と、SESSION専用変数の`@@global.x`は文の1238で、読み取りがどこにあっても — 死んだ分岐でもサブクエリでも0行の`SELECT`でも — 変わらない。スコープ表はサーバソース（`sql/sys_vars.cc`）から生成していて標準の変数を全部持つ。プラグインやコンポーネントの変数（`@@x.y`を含む）と修飾なしの`@@x`は判定せず、未知の名前はサーバ自身の1193に任せる。
