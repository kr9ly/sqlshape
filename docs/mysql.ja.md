...
- 行エイリアス（`INSERT ... VALUES (...) AS new [(名前リスト)]`、8.0.19）は、挿入される列——表の全列ではなく、そのINSERT自身のフィールド——を、名前リストがあれば位置対応で改名して、`ON DUPLICATE KEY UPDATE`に対象表と並べて見せる。両方が持つ修飾の無い名前は1052、名前リストの個数違いは1353、対象表と衝突するエイリアスは1066で、`VALUES(c)`はエイリアスと併用できる（いずれも実測）
