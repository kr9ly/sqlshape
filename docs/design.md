...
- 方言の口（裁定 2026-09-09）: vet が`analyze.Result` / `schema.Schema` / `pgparse`に直接依存しているので、ルートに`x/dialect`を切る。`Load(schemaSQL) (Analyzer, error)`、`Analyzer.Analyze(sql) (*Result, error)`、Go 型 ↔ SQL 型の対応表を方言が提供し、`schema.sql`の先頭宣言（`-- sqlshape: postgres 18` / `-- sqlshape: mysql 8.4`）で選ぶ。`Result`は方言非依存の形（型は名前と NULL 可否、出所はテーブル・列名、`facts.Facts`）に寄せる。PG 実装は`check/postgres/analyze`の薄い adapter、MySQL 実装は`mysql/`側。vet が pgparse に触る 2 箇所（ORDER BY の hazard、束縛の定数）は方言側へ。SQLite 等の第三の方言もこの口に足すだけで済む形にする（パーサは同じ手法 — 本物の文法からの機械抽出 — で、Lemon の文法なら parsegen に相当する生成器をもう 1 つ書く）
  ...
  - カタログの検算（裁定 2026-09-09）: mysqld オラクル（配布 tarball を`--initialize`で起こす）はカタログ生成のときに手元でだけ使い、生成物をコミットする。CI は検算しない（PG の`catalog/gen`と同じ扱い。tarball は 500MB 級で CI に載せる価値がない）。tarball は`~/.cache/sqlshape/mysql-<version>`に置く
    - マイグレーションの差分テスト（2026-09-15、実施）: `check/mysql/migrate`の`TestMigrateProbe`が、計画器の語彙（列型 12 種、生成列、キー、FK、AUTO_INCREMENT、CHECK、ビュー、トリガ、関数）からスキーマ A を乱数で生成し、変異（列の追加・削除・改名・拡幅・移動・NULL 可否、キー / FK / CHECK の付け外し、PK の既存列・新列への移動、表の追加・削除・改名、ENUM ラベル、ビュー・トリガ・関数の追加・削除・変更、生成式の変更）を 1〜3 個重ねて B を作り、`sqlshape apply`と同じ判定を実サーバで下す: Plan(A→B) の DDL を A を持つ mysqld に流して B の正準形と diff が空、その状態から再 Plan が空。変異は宣言（`-- @migrate`）も書く（左は今の名前、右はこれからの名前。1 オブジェクト 1 宣言になるよう、動かした対象は他の変異が触らない）。落ちた組は変異を削って最小化し、レポートに出す。seed 1 の 200 組がゲート。初回スイープ 11 seed 約 3,000 組で 7 クラスの順序問題を見つけて修正（`probe_findings_test.go`に固定）: FK の列に宣言キーを置くとサーバ製の暗黙インデックスが消えるので DROP INDEX と ADD は 1 文（1553）、参照されている列から PK を外すときも同じ、生成列が読む列の RENAME COLUMN は依存生成列の MODIFY を同じ文に（3108）、置換されるビューは追加列の後（1054）なのでビューは新規・置換ともに最後に依存順、新表の CREATE は既存表の新列・新キーの後（3734 / 6125）で既存表の新 FK は新表の後、型変更で落とす FK は改名後の表名（1146）。手作業の 2 巡目が 6 件で全部この 1 クラスだったのに対し、生成器は 1 回のスイープで倍近くを拾った。届いていない語彙: EVENT、既存表の AUTO_INCREMENT 値、文字集合・照合、パーティション、複合 FK、データ入り表（backfill・NOT NULL 追加の失敗）
    - 同じ probe を PostgreSQL 側にも（2026-09-15、`check/postgres/migrate/probe_test.go`）。語彙は PG のもの（型 11 種 + ENUM 型、IDENTITY、STORED 生成列、UNIQUE 制約と CREATE INDEX、FK、CHECK、COMMENT、ビュー、SQL 関数、トリガ関数 + トリガ）、列の移動は無く列順は notes として無視、判定は埋め込み PG。初回スイープ 11 seed 約 3,000 組で 10 クラス: トリガ関数の DROP は表の後（2BP01）、生成列が読む列の型変更は生成列を DROP / ADD で挟む（0A000）、新表の IDENTITY 列のシーケンスを二重に出さない（0A000）、列の追加・削除を生成列の依存順に（42703 / 2BP01）、消えるキーに乗る FK は先に落として後で戻す（PK の移動・表改名に伴う制約名の変更・参照表自体の削除、2BP01）と全表の FK を他の制約より先に、生成列の書き直しが落とす複合制約・インデックス・CHECK を戻し二重に足さない（42710）、書き直す生成列を読むビューは前後で作り直し（2BP01）、改名だけの式差は書き直さない、置換ビューは追加列の後（42703）で ENUM 再作成が落としたビューは一度だけ作る（42P07）、表の rename 宣言を列の rename より先に読む。2 巡目（同日）: 両 probe を行入り表（1 表 3 行、整数列は行番号、NULL 可の列は 3 行目が NULL）・変異 5 個まで・複合 FK に広げ、PG は domain・別 schema・bigserial・マテビュー、MySQL は EVENT・列の照合・INVISIBLE・ON UPDATE・接頭キーを足した。行を要する変異は `-- @migrate backfill` を自分で宣言する。見つかった追加の穴: MySQL は backfill が MODIFY NOT NULL の後（1138）・新列のキー / FK の後（1452）、新 AUTO_INCREMENT 列のキーは同じ ADD COLUMN に（1075）、全表の FK を他表のキーより先に落とす（1553）、`/*!80023 INVISIBLE */` の列テキストが `*/` を落としていた（ローダー）。PG は改名表のコメント、bigserial のシーケンスは表・列の改名に追従（42P01）。残る語彙の穴: パーティション、RLS、複合型、MySQL の既存表の AUTO_INCREMENT
