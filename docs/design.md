...
- MySQLを、PostgreSQLとは別の実装としてサポートする。初期リリースの範囲外。方言をまたぐ共通DSLは作らない（作った瞬間にSQLをDSLに翻訳させる形に戻る）。代わりにプラットフォームごとに、そのDBの構文と型規則を実装したアナライザーと、そのDBのdumpを使うマイグレーション支援を持つ。共有するのはテンプレートの展開、`go/analysis`のフロントエンド、Go側の型照合の枠組み
  - ライセンス（裁定 2026-09-09）: MySQLサーバはGPLv2で、Apache 2.0のsqlshapeと同じ成果物には混ぜられない（libpg_queryの形が成立したのはPostgreSQL Licenseが寛容だったから）。文法・字句解析器・`strings/`を切り出したwasmは明確にGPLの派生物なので、MySQL対応は別モジュール・別リポジトリの`sqlshape-mysql`（vettoolのバイナリ）としてGPLv2で出す。ユーザーのアプリにリンクされるランタイム`sqlshape`パッケージはApacheのまま、パーサはツールの実行時にしか要らないので、GPLがユーザーのコードに及ばない（開発ツールがGPLなのはgccと同じ）。共有部分（analyzeの枠組み・expand・obligation・vetのフロントエンド）はApacheで公開し、GPL側がimportする — Apache→GPLの一方向は互換。関数表など「事実」のTSVはApache側にも置けるが、線引きの争いを避けるためまとめてGPL側に置く。退けた案: パーサを別プロセスにしてパイプで木を渡す（FSFは密なデータ交換を結合とみなし得るとしていて灰色、構造も歪む）、Apache 2.0の互換パーサ（TiDB parser / Vitess）を使う（原則を捨てる）、抽出を事実に限定して文法を手書きする（一番品質が欲しい文法が手書きになる）
