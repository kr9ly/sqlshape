...
#### トリガ・ルーチンの独自コードにGoの名前を与える（裁定2026-09-13）

決めたこと。正はスキーマの`-- sqlshape: error <code> = <Name>`注釈であり、Goはそれを`sqlshape.Error(code)`で写すだけ（`var OrderTooLarge = sqlshape.Error("30001")`）。`vet`が両方向を見る——宣言のコードがスキーマの宣言どおりの、まさにそのNameのものであること、パッケージの文が起こしうる名前付き失敗モードのすべてに宣言が(自分のパッケージか、参照が届く他パッケージに)あること。ランタイムの`Violates`は`~string`を取るだけで、渡された値がFailureか生の文字列かを区別せず、スキーマも読まない——`sqlshape.Error(code)`が返す`Failure`はコードそのものを運ぶ値でしかなく、Nameを解決したり登録したりする経路は無い。expect行はコードでもNameでも書け、両方書いても片方が余らない（`Violation.Name`が付いているときだけ、そのNameも満たされたキーに数える）。

理由。「正は1つ、複製は写しでしかない」という他の裁定（型のPG名↔Goの対応、enumラベルのFactなど）と同じ形に揃えた。ランタイムにNameからコードへ解決させる方式（登録テーブルをどこかに持つ）は、ランタイムがスキーマに依存する新しい経路を作ってしまい、「ランタイムはschema.sqlを読まない」という既存の不変を壊す。Go側にNameとコードを両方持つ独自の宣言方式（`sqlshape.NamedError{Name, Code}`のような値をvetでなく実行時にも参照する形）も検討したが、`Failure`はただの`~string`のままにして`Violates[K ~string]`の型引数で受けるほうが、コードの読み書き・比較・switchが素朴な文字列のまま扱えて、ランタイム側に一切の分岐が要らない。この裁定はMySQLの`SIGNAL`キーにも及ぶ——先の裁定（下記MySQLトリガの項）でNameをキーの別名にしないと決めたのは「複数のSIGNALが同じ`MYSQL_ERRNO`を別の意味で使いうる」ことを懸念してのものだったが、Nameをキーの別名にする経路自体を作らず、Goの`sqlshape.Error(code)`がコードだけを運ぶ値である以上、その懸念は生じない（`Violates(err, OrderTooLarge)`は結局`Violates(err, "30001")`と同じ呼び出しでしかない）。
...
受け入れた冗長と限界。列順の差は許容して注記する（PGは列を途中に挿せないので、弾くとapplyが二度と通らなくなる）。renameした列を含む制約・FKはDROP + ADDで出る（正しいが再検証が走る。痛くなったらprops比較にrename写像を当てる）。domainの基底型・rangeのサブタイプ・INHERITS・OF typeの変更は注記のみ。行を持つ表のパーティションキーの変更と、列に使われている複合型の属性の型変更は、無損失のDDLが無いので問題として止める（注記だとapplyは通り、verify-schemaが永久に差分を報告する）。pg_dumpは利用者環境のもの（embedded PGにクライアントツールは同梱されない。pg_catalog直読みは必要になったら）。

棄てた案。sqldefのラップ。pg_catalog直読みによる逆ロード（pg_dumpの方が正準形が得やすい）。

スコープは運用まで（裁定2026-09-16）。パーティションの一覧はスキーマの一部なので、列と同じ資格で比較・計画・検証する。Skeemaの`partitioning=keep`のように「時系列RANGEのローテーションは運用だから差分を見ない」と切ると、verify-schemaの立場からは検出できないドリフトを設計で作ることになる。こちらの答えは、行を落とすDROP PARTITIONは列のDROPと同じく宣言を要求し、行を動かす変更（MySQLのREORGANIZE、PGのDETACH + ATTACH）はDDLとして導き、無損失のDDLが無い差分は問題で止める。無視はしない。この先にあるのは、後から表をパーティション化する手順（PGはDEFAULTパーティションにATTACHしてから範囲ごとに行を移す）と、ローテーションの規則そのものを宣言してパーティション集合を導くこと。

#### 忠実度の測り方（裁定2026-09-15）

検査器と同じく本物のサーバをオラクルにするが、問う内容が違う。検査器は「この文をサーバはどう判定するか」を聞き、計画器は「この DDL をサーバは受け入れ、着いた先が目的のスキーマか」を聞く。`check/{postgres,mysql}/migrate`の`TestMigrateProbe`が、計画器の語彙からスキーマAを乱数で生成し、変異を1〜5個重ねてBを作り（変異は自分の`-- @migrate`宣言も書く）、Plan(A→B)のDDLを3行ずつ入ったAを持つ実サーバに流す。合格は三つ: 拒まれない、読み返した正準形がBと一致する（PGは列順を除く）、そこから再Planが空。落ちた組は変異を削って最小化し、直した穴は`probe_findings_test.go`に最小の組で固定する。

網羅の単位。検査器のコーパスはサーバ自身の回帰テストで、網羅は所与だった。乱数生成には網羅の単位が無いので、計画器の入力に置く。計画器の入力はdiffの出力だから、diffが出し得る変更の全種（`(Op, Kind, Field)`の三つ組。`+ table`、`~ column type`、`~ constraint deferrable`…）をアルファベットと呼び、`diff.Alphabet()`がCompareと同じProps関数から機械的に列挙する（キーを写して二重に持たない。無条件に増えたキーは自動で拾い、新しい条件の下のキーは代表スキーマの拡張が要る）。ゲートはseed 1の200組で、踏んだ集合と理由付きの既知未到達の和がアルファベット全体に一致すること。既知未到達に置けるのは「計画器がDDLを書かず問題として止める差分」と「構造的に現れない三つ組」だけで、生成器の怠けは置けない。diffにプロパティが増えると生成器が追従するまでゲートが落ちる。組み合わせと順序は乱数に任せる、という分担。乱数だけだと「seed 1の200組がたまたま全変異を引く」ことに網羅が依存し、変異を1つ足すとPRNGの列がずれて既存の網羅が落ちる（3巡目で実際に起きた）ので、各変異を単独で適用した組を毎回1つずつ作るdirected passを足し、乱数の組との和で集計する。PostgreSQLはprobeを17と18の両方で回す（`TestMigrateProbe18`。18専用の語彙——VIRTUAL生成列、NOT ENFORCED、WITHOUT OVERLAPS、PERIOD——は18のゲートで踏む）。

数字（2026-09-16、5巡目）。PostgreSQLはアルファベット103のうち17のゲートで94、18のゲートで98を踏む。未到達は、無損失のDDLが無く問題で止める差分4（INHERITS、OF type、domainの基底型、rangeのサブタイプ。以前は注記だったが、注記だとapplyが通りverify-schemaが永久に差分を出すので問題に変えた）、構造的に現れないもの1（pg_dumpは名前付きUNIQUEを常に制約で出すので`~ index unique`は無い）、17のゲートではさらにPG 17が構文を拒む18の語彙4。MySQLは48を全部踏む（パーティションはRANGE / LIST / COLUMNS / HASH / KEY / LINEAR / サブパーティションまで構造にし、Textのフォールバックは無くした）。行を持つ表のパーティションキーの変更は、旧表をRENAMEして目標の形を新しく作り、宣言されたパーティションを付け、`INSERT ... SELECT`でPGのrouterに全行を配らせ、旧表を落とす手順で書く（空→X、X→空、X→Yとも。コメント・トリガ・ポリシー・RULE・被参照FK・sequenceを付け直す。bigserialのsequenceはOWNED BY NONEで退避して新表に再所有、identityは旧sequenceを退避改名し`OVERRIDING SYSTEM VALUE`で入れて`setval`で続きから）。固定した計画器の穴は`probe_findings_test.go`のPG 35本、MySQL 21本。ほぼ全部がDDLの順序（FKが乗るキー、生成列の依存、PK不在の窓、スキーマの作成と削除、ポリシーとRULEが読む列の型変更）で、依存グラフへの作り替えはせず種類別の固定順の穴埋めで収束している。

届かないもの。生成器のモデル外にある実世界のスキーマの形（レガシーな綴り、拡張型、巨大な表）と、データ依存の失敗（backfillの値、ロック時間）。前者は利用者の`schema.sql`が持ち込むもので、applyの終点比較が最後の網。パーティションは語彙に無く、次の巡で足す。

棄てた案。実世界コーパス（regressの`CREATE TABLE`群をAにする）はdumpの往復に乗らないオブジェクトが多く、変異の網羅にもならない。計画器の行カバレッジをゲートにするのは、未到達行が語彙の穴か死んだ分岐か区別できず宣言の単位として粗い（補助の数字としてなら可）。
...
事実の正しさは実サーバで測る（2026-09-16）。`x/stmtprobe`がスキーマと行と文を生成し、両方言のアナライザーが出したFactsの主張——述語が返る全行で成り立つ、列が既知の値に固定されている、高々1行、書き込みが格納する値——をサーバの返した行で反証する（検査器の型をembedded PGで、マイグレーションの計画を正準形一致で測るのと同じ位置づけ。マイグレーションのprobeと同じく生成器・判定器・報告を方言中立に一つ書き、ドライバだけ方言ごと）。初回で両方言に1件ずつ見つかった（MySQLの1093 / 1443、PGのJOIN ON内サブクエリの外部参照）。同じ生成器で失敗モードも測る: 全種類の制約を持つスキーマへの書き込みを走らせ、サーバが出した制約エラーは予測した違反のどれかにランタイムの`Violates`で一致しなければならない（PGが「毎回失敗する」noteで言う確定失敗も予測に数える）。MySQLの本体はサーバのCREATE時判定と（`check/mysql/internal/analyze`のbody probe）、文の型とエラーはmysql-testコーパス1,281ファイルの並走と（同corpus probe、PGのregress probeの対）で測る。コーパスは初回44,342文一致・4,605不一致をベースラインとして固定し、以後の巡で削る（PGは5,282から47まで削った）。
...
- 方言の口（裁定 2026-09-09）: vet が`analyze.Result` / `schema.Schema` / `pgparse`に直接依存しているので、ルートに`x/dialect`を切る。`Load(schemaSQL) (Analyzer, error)`、`Analyzer.Analyze(sql) (*Result, error)`、Go 型 ↔ SQL 型の対応表を方言が提供し、`schema.sql`の先頭宣言（`-- sqlshape: postgres 18` / `-- sqlshape: mysql 8.4`）で選ぶ。`Result`は方言非依存の形（型は名前と NULL 可否、出所はテーブル・列名、`facts.Facts`）に寄せる。PG 実装は`check/postgres/analyze`の薄い adapter、MySQL 実装は`mysql/`側。vet が pgparse に触る 2 箇所（ORDER BY の hazard、束縛の定数）は方言側へ。SQLite 等の第三の方言もこの口に足すだけで済む形にする（パーサは同じ手法 — 本物の文法からの機械抽出 — で、Lemon の文法なら parsegen に相当する生成器をもう 1 つ書く）
  ...
  - サーバ設定の宣言（裁定 2026-09-11、実施同日）: 判定を変えるサーバ変数は、版と同じくschema.sqlが宣言する。構文は方言非依存で`-- sqlshape: server <var> = <value>`（1行1変数、値はSQLの文字列リテラルか裸のトークン、`x/dialect.Settings`が読む）、受け付ける変数の列挙は方言ごと（知らない変数・不正な値はスキーマのProblem）。MySQLが読むのは`sql_mode`と`lower_case_table_names`の2つで、宣言が無ければ「8.4を初期化したままのサーバ」（sql_modeの既定、lower_case_table_names=0）を規定とする。`sql_mode`の語彙・ビット・組み合わせモード（`ANSI` / `TRADITIONAL`）の展開は`x/sqlmode`（sys_vars.ccの`sql_mode_names` / `expand_sql_mode`の写し。ルートに置くのは、GPLの検査器とApacheのランタイム / mysqltestの両方が同じ表を読むため — `x/placeholder`と同じ位置づけ）。検査器が従うのは、字句・文法のビット（`ANSI_QUOTES` / `PIPES_AS_CONCAT` / `IGNORE_SPACE` / `NO_BACKSLASH_ESCAPES` / `HIGH_NOT_PRECEDENCE`、shimのTHDにそのまま渡す — `||`は字句解析器が`OR_OR_SYM` / `OR2_SYM`を選ぶので変換層の分岐は不要だった）、`ONLY_FULL_GROUP_BY`（グループ検査の有無）、`STRICT_TRANS_TABLES` / `STRICT_ALL_TABLES`（`Item_str_func`のNULL可否、1048の失敗モード）、`REAL_AS_FLOAT`（DDLの`REAL`）、`NO_UNSIGNED_SUBTRACTION`（減算のunsigned）。残りの語（日付・0除算・エンジン置換・パディング）は実行時にしか効かないので受理して素通し。`lower_case_table_names`は1なら表名・ビュー名を小文字に正規化して保持（サーバの報告と同じ）、2なら宣言どおりの綴りで大文字小文字を無視して照合、0は区別する。サーバとの突き合わせ: `mysqltest.Start`が宣言をそのままmysqldの起動オプションに渡す（`lower_case_table_names`は初期化後に変えられないのでテンプレートを値ごとに分ける。知らない変数はmysqldが起動を拒む）ので、オラクルもアプリのテストも宣言と同じ世界で走る。本番の接続に対しては`mysql.Verify(ctx, db, schemaSQL)`が`@@SESSION.sql_mode`（DSN / プールで上書きされうるのでSESSION）と`@@lower_case_table_names`を読み、宣言とビットで比べて不一致を返す。退けた案: `SET`文をschema.sqlに書かせる（DDLに混ざり、適用対象になる）、`-- sqlshape: mysql 8.4 sql_mode=...`と版の行に載せる（変数が増えると読めない）、mysqltestに宣言の正規表現を複製する（表が二重になる）
  - マイグレーション（裁定 2026-09-11、実施中）: PG と同じ 3 部構成を `check/mysql/dump` / `diff` / `migrate` に置き、`sqlshape diff` / `apply` / `verify-schema` は schema.sql の宣言で方言を選ぶ（cli を方言ごとの driver に切る）。正準形は「その文を適用したサーバに `SHOW CREATE TABLE` / `SHOW CREATE VIEW` で読み返した DDL」を自前のローダー（`check/mysql/internal/schema`）で読んだモデル。両側（目標の schema.sql と稼働 DB）を同じ経路で読むので、比較されるのは書き方でなく MySQL が保持している定義になる（PG が両側を pg_dump で読むのと同じ位置づけ。mysqldump は要らない。`SHOW CREATE TABLE` の出力をローダーがそのまま読めることは実測済み: 列 9 / キー 3 / CHECK 2、`/*!80016 NOT ENFORCED */` も読む）。読み返しで落とすもの: `AUTO_INCREMENT=N` のカウンタ、ビューの `DEFINER`。正準化に使うサーバは、`-db` があるときはその稼働サーバ上の一時データベース（`sqlshape_scratch_<乱数>` を作って適用し、読み返して落とす。同じサーバ版・同じ設定で正規化されるので、別のローカル mysqld より正しい。CREATE / DROP DATABASE 権限が要る）、`-from`（テキスト同士）とテストでは PATH の mysqld（mysqltest、宣言した設定で起動）。宣言した `sql_mode` は一時 DB のセッションに `SET SESSION` で入れ、`lower_case_table_names` はサーバの値が宣言と違えば止める。差分はモデルの上で、表（エンジン・文字集合・照合・コメント）、列（型の正準綴り・NULL 可否・既定値・AUTO_INCREMENT・ON UPDATE・生成式と STORED・コメント・INVISIBLE・照合・位置）、キー（種別・列と接頭長・コメント）、外部キー（列・参照・ON DELETE / UPDATE）、CHECK（式・ENFORCED）、ビュー（サーバが正規化した本文・ALGORITHM・CHECK OPTION）。列の順序は PG では直せないので注記だったが、MySQL は `MODIFY ... AFTER` で直せるので差分として出す。トリガ・プロシージャ・イベントはローダーが読まないので比較しない（docs に明記）。seed 表は MySQL のローダーに無いので今回は範囲外。DDL の生成は MySQL 構文: drop（参照している外部キー → ビュー → 表 / 列 / キー / CHECK）、宣言された rename（`RENAME TABLE` / `RENAME COLUMN`）、alter（`MODIFY COLUMN` の全定義 + `FIRST` / `AFTER`、表オプション、キーと FK と CHECK は DROP + ADD）、add（正準テキストの `CREATE TABLE` を FK 依存順に、ビューはその後）、backfill。`-- @migrate` の文法は PG と同じ（`enum` は MySQL では列を名指す: `enum orders.status: drop 'x' using 'y'` → UPDATE してから MODIFY）。verify は「稼働 DB の正準テキスト + DDL」を一時 DB に適用して読み返し、目標と比べる。apply: MySQL の DDL はトランザクションにならない（暗黙コミット）ので 1 文ずつ実行し、失敗した文で止めてどこまで進んだかを報告する。`-no-transaction` は MySQL では意味を持たない。退けた案: mysqldump を両側に使う（バイナリ依存が増え、`SHOW CREATE` と同じ情報しか出ない）、information_schema を直接読んでモデルを組む（ローダーの二重実装になる。正準 DDL を読ませれば一つで済む）、ローカル mysqld を必ず要求する（配布バイナリが無いので PG の embedded とは事情が違う）
  - トリガとストアドルーチン（裁定 2026-09-11 セッション末、実施）: PostgreSQLのPL/pgSQL本体解析と同じ位置づけで**必須**とし、後回しにしない（docs/checks.mdは「失敗モードはトリガと呼び出す関数の中まで計算する」と約束しており、MySQLだけトリガ本体を読めないままだと`SIGNAL`の失敗モードが漏れ、本体のDMLが義務の判定を逃れる）。本体はスキーマごとに1回解析してキャッシュし（`AnalyzeTrigger` / `AnalyzeRoutine`、`BodyResult`）、結果をルーチンの呼び出し側・CALL・トリガを起こす文のそれぞれが読む。SIGNALのキーは`SET MYSQL_ERRNO`があればその10進表記、無ければSQLSTATE（PostgreSQLの`RAISE ... USING ERRCODE`と同じ位置づけ。runtime側の`mysql/errors.go`も同じ規則でサーバのエラーを読み戻す）。`-- sqlshape: error <key> = <Name>`はPGの`error`注釈と同じ構文で足した。当初はNameをキーの別名にしない設計だった（`Violation.Key()`はConstraintしか見ず、Nameは`describeViolation`の説明文の装飾に留める）——理由は、`MYSQL_ERRNO`は同じ値でも複数のSIGNALが別の意味で使う可能性があり、キーをNameで直接上書きすると`mysql.Violates`のランタイム側の照合（サーバのSQLSTATE / 番号から一意に引ける）と食い違う経路が生まれることを懸念したため。この懸念は裁定2026-09-13（上記「トリガ・ルーチンの独自コードにGoの名前を与える」）で解消した: NameはGoの`sqlshape.Error(code)`宣言の識別子名としてvetが照合するだけで、キーそのものの別名としてランタイムに登録されることは無い（`Violates`はスキーマもNameも知らず、渡された値のコードだけを見る）ので、複数のSIGNALがコードを共有してもNameが誤って解決される経路自体が存在しない。expect行・`mysql.Violates`はNameでも書けるようになったが、それは`Violation.Name`が一致するキーの別表記として認められるだけで、キー自体（`Violation.Key()`）は変わらずSIGNALのコードのままである。`DECLARE ... HANDLER FOR`はブロック単位で吸収し（SQLEXCEPTION / SQLWARNING / NOT FOUND / SQLSTATE / 番号 / 条件名）、値なしRESIGNALで戻す設計はPL/pgSQLの`EXCEPTION`ブロックと同型。`CALL`の結果列は「本体のINTO無しSELECTが0個なら列無し、1個ならその列、複数あって形が全部一致すれば1つの列リスト、割れていれば検査器自身のエラー」とし、mysqld自身の拒否ではなく検査器の判断にした——mysqldは実行時にどちらの結果集合を返すか経路依存で決めるだけで、CREATE時にもCALL時にも形の一致を検査しないため、静的に扱うには検査器側で裁定するしかなかった。マイグレーションはトリガ・ルーチンをDROP + CREATEで置き換える（MySQLに`CREATE OR REPLACE TRIGGER`が無いのはテーブル同様の制約で、表・ビューと同じ依存順の裁定に従う: トリガのDROPは表のDROPより前、ルーチンのCREATEはビューより前、トリガのCREATEはbackfillの後）。`mysqlparse.Split`の複合文分割（裁定2026-09-09）は変えず、DELIMITER非依存の`;`拡張方式のままトリガ・ルーチン本体にも通用した——本体を字句的に読むだけの分割なので、意味解析側の変更だけで済んだ
  ...
  - カタログの検算（裁定 2026-09-09）: mysqld オラクル（配布 tarball を`--initialize`で起こす）はカタログ生成のときに手元でだけ使い、生成物をコミットする。CI は検算しない（PG の`catalog/gen`と同じ扱い。tarball は 500MB 級で CI に載せる価値がない）。tarball は`~/.cache/sqlshape/mysql-<version>`に置く
    - マイグレーションの差分テスト（2026-09-15、実施）: PG と同じ probe を`check/mysql/migrate`に置く。測り方と数字は「マイグレーションは自前のdiff / apply」の「忠実度の測り方」に一本化した。MySQL 固有の判定は正準形が「その文を適用した mysqld に`SHOW CREATE`で読み返した DDL」であること、列の移動があること、DDL が文ごとに commit されるので apply が文単位で進むこと
