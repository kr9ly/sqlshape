...
- MySQLを、PostgreSQLとは別の実装としてサポートする。初期リリースの範囲外。方言をまたぐ共通DSLは作らない（作った瞬間にSQLをDSLに翻訳させる形に戻る）。代わりにプラットフォームごとに、そのDBの構文と型規則を実装したアナライザーと、そのDBのdumpを使うマイグレーション支援を持つ。共有するのはテンプレートの展開、`go/analysis`のフロントエンド、Go側の型照合の枠組み
  - パーサ（裁定 2026-09-09）: MySQLサーバのソースにある文法定義`sql/sql_yacc.yy`（bison）から、C++のアクションを剥がして文法だけを抽出し、そこからパーサを生成する。文法は版ごとに本物を使う（PGでlibpg_queryの版を積むのと同じ形。`-- sqlshape: mysql 8.4`）。生成したパーサは規則名と子要素だけの素の構文木（CST）を作り、sqlshape用のASTへの変換は手で書く。字句解析器はMySQLの`sql_lex.cc`がサーバ内部（THD・文字集合）と絡んで切り出せないので書き直す（バージョンコメント`/*!80000 ... */`、文字集合導入子`_utf8mb4'...'`、区切り文字が要点）。予約語表は`sql/lex.h`から取る
  - パーサの生成手段は未決。候補は2つ: ①抽出した文法をgoyaccに通してpure Goで持つ（ツールチェーン不要、LALR(1)と`%prec`の解決はbisonと同じアルゴリズムだが、`%union`・中間アクション・`%destructor`等のbison拡張の変換が要る）、②bisonでCのパーサを生成しlibpg_queryと同じくwasmにする（bisonの解決をそのまま使えるが、字句解析器もCで書くことになる）。スパイクで決める: 文法抽出 → 生成 → MySQLのテストスイート（`mysql-test/t/*.test`）の文をどれだけ受理するか
