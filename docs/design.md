...
RDBMSを使うアプリケーションに要るのは4つ。SQLの構文と型の検査、型安全な値のバインド、結果の構造体へのマッピング、スキーマから導けるマイグレーション。既存ツールがこの4つを同時に満たせないのは、前の3つは「SQLが正」、最後は「アプリの型定義が正」と真実の源が割れているからで、両方やろうとするとSQLをDSLに翻訳させることになる。sqlshapeは真実の源を`schema.sql`とSQLそのものに置き、アプリの型は照合される側にする。
