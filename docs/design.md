...
網羅の単位。検査器のコーパスはサーバ自身の回帰テストで、網羅は所与だった。乱数生成には網羅の単位が無いので、計画器の入力に置く。計画器の入力はdiffの出力だから、diffが出し得る変更の全種（`(Op, Kind, Field)`の三つ組。`+ table`、`~ column type`、`~ constraint deferrable`…）をアルファベットと呼び、`diff.Alphabet()`がCompareと同じProps関数から機械的に列挙する（キーを写して二重に持たない。無条件に増えたキーは自動で拾い、新しい条件の下のキーは代表スキーマの拡張が要る）。ゲートはseed 1の200組で、踏んだ集合と理由付きの既知未到達の和がアルファベット全体に一致すること。既知未到達に置けるのは「計画器がDDLを書かず注記か問題として返す差分」と「構造的に現れない三つ組」だけで、生成器の怠けは置けない。diffにプロパティが増えると生成器が追従するまでゲートが落ちる。組み合わせと順序は乱数に任せる、という分担。

数字（2026-09-16、4巡目）。PostgreSQLはアルファベット103のうち92を踏み、未到達11はDDLを書かない差分5（INHERITS、OF type、domainの基底型、rangeのサブタイプは注記、行を持つ表のパーティションキーは問題）、probeのサーバがPG 17なので現れないPG 18の語彙4、構造的に現れないもの2（pg_dumpは名前付きUNIQUEを常に制約で出すので`~ index unique`は無い、`LANGUAGE internal`の関数は追跡外なので`~ function window`は無い）。MySQLは48を全部踏む。4巡で固定した計画器の穴は`probe_findings_test.go`のPG 24本、MySQL 16本。ほぼ全部がDDLの順序（FKが乗るキー、生成列の依存、PK不在の窓、スキーマの作成と削除、ポリシーとRULEが読む列の型変更）で、依存グラフへの作り替えはせず種類別の固定順の穴埋めで収束している。
