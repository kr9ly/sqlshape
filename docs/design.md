...
数字（2026-09-15、3巡目）。PostgreSQLはアルファベット102のうち90を踏み、未到達12は注記のみの差分6（INHERITS、パーティション2、OF type、domainの基底型、rangeのサブタイプ）、probeのサーバがPG 17なので現れないPG 18の語彙4、構造的に現れないもの2（pg_dumpは名前付きUNIQUEを常に制約で出すので`~ index unique`は無い、`LANGUAGE internal`の関数は追跡外なので`~ function window`は無い）。MySQLは48のうち47を踏み、未到達1は問題のみで返すパーティション差分（`~ table partitioned`）。3巡で固定した計画器の穴は`probe_findings_test.go`のPG 18本、MySQL 15本。ほぼ全部がDDLの順序（FKが乗るキー、生成列の依存、PK不在の窓、スキーマの作成と削除、ポリシーとRULEが読む列の型変更）で、依存グラフへの作り替えはせず種類別の固定順の穴埋めで収束している。
