...
sqlshapeは、Goのコードに書いた素のSQLを`go vet`で検査するツールである。ORMやクエリビルダを挟まずにSQLをそのまま書き、結果を受け取る構造体とパラメータの構造体がSQLの中身と食い違っていないかを、コンパイル時に静的に確かめる。
...
- 生成ではなく検査。すべての文を`schema.sql`と照らして解析し、SELECTの列が受け取り側の構造体と揃っているか、`{{.X}}`に渡す型が合っているか、NULLになりうる列をポインタで受けているか、`One`と宣言した文が本当に1行しか返さないか、INSERTやUPDATEが違反しうる制約を宣言しているかを確かめる。結果はエディタのProblemsペインに出る。
- 素のSQLで、SQLインジェクションが起きない。テンプレートはGoの`text/template`で、`{{.X}}`は必ず`$n`のプレースホルダになり、文字列としてSQLに埋め込まれることはない。`{{if}}`や`{{range}}`の分岐は全組み合わせが展開されて検査される。実行時も、検査器が見ていない形のSQLは実行を拒否する。
- `schema.sql`だけがスキーマの定義。検査器はこれを読み、テストは`pgtest`でこれを適用した本物のPostgreSQLに対して走り、マイグレーションの差分も`sqlshape diff`がここから導く。ビュー・関数・ドメイン・複合型・行レベルセキュリティ・seed済みのlookupテーブルも検査対象なので、アプリケーションには生のテーブルではなく型のついたAPIを見せられる。
