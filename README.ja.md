...
- 生成ではなく検査。すべての文を`schema.sql`と照らして解析し、SELECTの列が受け取り側の構造体と揃っているか、`{{.X}}`に渡す型が合っているか、NULLになりうる列をポインタで受けているか、`One`と宣言した文が本当に1行しか返さないか、INSERTやUPDATEが違反しうる制約を宣言しているかを確かめる。違反はすべて`go vet`の診断として、コードを実行する前に報告される。
