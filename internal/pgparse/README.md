...
1. Parser. `cd internal/pgparse/wasm && nix-shell -p emscripten protobuf protoc-gen-go --run
   ...
   ./internal/pgparse` — `TestCorpus` needs the previous version's regress corpus and fails on a
...
2. Oracle. Add the release to `binaries` in `internal/oracle/oracle.go` (embedded-postgres must
   ship it). Check: `go test ./internal/oracle`.
3. Catalog. `go run ./internal/catalog/gen -pg <N>` then `-pg <N> -all-ext` writes
   `internal/catalog/data/<N>/`. Check: `go test ./internal/catalog`.
4. Regress corpus and baseline. `internal/analyze/testdata/tools/fetch-regress.sh REL_<N>_<x>`
   (the release the oracle runs), then `SQLSHAPE_PG=<N> go test ./internal/analyze -run
