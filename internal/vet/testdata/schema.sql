...
-- a materialized view with a unique index: REFRESH ... CONCURRENTLY is possible
CREATE MATERIALIZED VIEW plan_counts AS
SELECT plan, count(*) AS n FROM subscriptions GROUP BY plan;
CREATE UNIQUE INDEX plan_counts_plan_idx ON plan_counts (plan);
...
-- a boolean and a lone smallint column for the reverse DTO mapping (dto package): kept off
-- every other table so nothing else notices it
CREATE TABLE flags_test (
    id       bigint PRIMARY KEY,
    active   boolean NOT NULL,
    tiny     smallint NOT NULL,
    computed int GENERATED ALWAYS AS (tiny + 1) STORED,
