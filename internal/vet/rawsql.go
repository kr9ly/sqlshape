...
func checkRawSQL(pass *analysis.Pass, calls []*ast.CallExpr) {
	...
	for _, call := range calls {
		...
		if mode == "forbid" {
			if !allowed {
				pass.Reportf(call.Pos(), "sqlshape: %s.%s executes SQL outside sqlshape; with -raw-sql=forbid every statement goes through sqlshape.Query / One / Copy (or list the package in -raw-sql-allow)", fn.Pkg().Name(), rc.method)
			}
			continue
		}
