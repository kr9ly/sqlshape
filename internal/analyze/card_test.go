...
func TestCardinality(t *testing.T) {
	...
	cases := []struct {
		...
		{sql: "SELECT id FROM users WHERE id = random()", why: "users"},
		// other single shapes
		{sql: "SELECT count(*) FROM orders", one: true},
		{sql: "SELECT max(total) FROM orders WHERE user_id = $1", one: true},
		{sql: "SELECT count(*) FROM orders GROUP BY user_id", why: "GROUP BY"},
		...
		{sql: "SELECT n FROM order_stats WHERE user_id = $1", why: "view order_stats: GROUP BY"},
