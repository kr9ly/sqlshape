...
func TestViolations(t *testing.T) {
	...
	cases := []struct {
		...
		{sql: "INSERT INTO orders (user_id, total) VALUES ($1, 10) ON CONFLICT (user_id, note) DO NOTHING",
			want: "23502 orders.user_id@1, 23503 orders_user_id_fkey, 23505 orders_pkey, 23505 orders_uid_active, 23514 orders_total_check, P0401 P0401"},
		{sql: "INSERT INTO orders (user_id, total) VALUES ($1, 10) ON CONFLICT ON CONSTRAINT orders_pkey DO UPDATE SET total = 5, status = 'paid'",
			want: "23502 orders.user_id@1, 23503 orders_user_id_fkey, 23505 orders_uid_active, 23505 orders_user_note_key, 23514 orders_total_check, P0401 P0401"},
		...
		{sql: "INSERT INTO orders (user_id, total) SELECT id, 1 FROM users",
			want: "23503 orders_user_id_fkey, 23505 orders_pkey, 23505 orders_uid_active, 23505 orders_user_note_key, 23514 orders_total_check, P0401 P0401"},
