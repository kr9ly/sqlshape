...
func TestAnalyze(t *testing.T) {
	...
	cases := []struct {
		...
		{"SELECT count(*), count(email) c, 1, 'x', 1.5, NULL, id = 1 FROM users", []string{"count(*) bigint", "c bigint", "1 bigint(1)", "'x' varchar(1)", "1.5 decimal(2,1)", "NULL null null", "id = 1 bigint(1)"}, nil},
		...
		{"SELECT LENGTH(name), UPPER(name), CONCAT(name, email), ABS(total), ROUND(total), ROUND(total, 1), FLOOR(total), CEIL(u.id), SQRT(u.id), NOW(), CURDATE(), CURTIME(), UNIX_TIMESTAMP(), YEAR(created_at), DATE(created_at), DATEDIFF(created_at, NOW()), DATE_FORMAT(created_at, '%Y'), JSON_EXTRACT('{}', '$'), MD5(name), UUID(), LAST_INSERT_ID(), HEX(u.id), CONV(u.id, 10, 16), ISNULL(email), CHAR_LENGTH(email) FROM users u JOIN orders o ON o.user_id = u.id", []string{"LENGTH(name) bigint", "UPPER(name) varchar", "CONCAT(name, email) varchar null", "ABS(total) decimal", "ROUND(total) decimal(10,0)", "ROUND(total, 1) decimal", "FLOOR(total) decimal(10,0)", "CEIL(u.id) bigint unsigned", "SQRT(u.id) double null", "NOW() datetime", "CURDATE() date", "CURTIME() time", "UNIX_TIMESTAMP() ? null", "YEAR(created_at) year null", "DATE(created_at) date", "DATEDIFF(created_at, NOW()) bigint null", "DATE_FORMAT(created_at, '%Y') varchar null", "JSON_EXTRACT('{}', '$') json null", "MD5(name) varchar", "UUID() varchar", "LAST_INSERT_ID() bigint unsigned", "HEX(u.id) varchar", "CONV(u.id, 10, 16) varchar null", "ISNULL(email) bigint(1)", "CHAR_LENGTH(email) bigint null"}, nil},
		// aggregates
		...
		{"SELECT id, EXISTS (SELECT 1 FROM orders WHERE nope = 1), id IN (SELECT user_id FROM orders), (SELECT 1) FROM users", []string{"id bigint unsigned", "EXISTS (SELECT 1 FROM orders WHERE nope = 1) bigint(1)", "id IN (SELECT user_id FROM orders) bigint(1)", "(SELECT 1) ? null"}, nil},
