...
func wrapErr(err error) error {
	...
	case 1442, 1172, 1416: // a trigger (or a called routine) writing a table already in use
		// up the invoking statement's chain; SELECT ... INTO with more than one row; a
		// geometry of another type than the spatial column's -- the checker's own model
		// (violations.go, call.go, geom.go) has no constraint name for any of them, so it
		// keys them by their own error number (Constraint: strconv.Itoa(code)), and
		// Violates(err, "1442" / "1172" / "1416") matches that key back here.
