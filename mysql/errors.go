...
func wrapErr(err error) error {
	...
	case 1442, 1172: // a trigger (or a called routine) writing a table already in use up
		// the invoking statement's chain; SELECT ... INTO with more than one row -- the
		// checker's own model (violations.go, call.go) has no constraint name for either,
		// so it keys them by their own error number (Constraint: strconv.Itoa(code) in
		// body.go), and Violates(err, "1442"/"1172") matches that key back here.
