...
(`schema.Schema.notifyNotNullChange`, `check/postgres/schema/schema.go`). The hook
...
- `testdata/queries/*.sql` + `.golden`: goldens come from the real PG (`go test ./check/postgres/analyze -update`);
