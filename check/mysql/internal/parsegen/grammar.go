...
// ViewsGo is the Go source of the typed views: one struct per parse-tree class the actions
// build, with an accessor per constructor parameter name, so that an analyzer reads
// `AsPTQuerySpecification(v).OptWhereClause()` instead of positions.
func ViewsGo(pkg string, version string, alts []Alt, names *Names) string {
	type class struct {
		name   string
		params []string
	}
	...
	for _, a := range alts {
		if a.Kind != ActNew || a.Class == "" {
			continue
		}
		params := names.ParamNames(a.Class, len(a.Args), HasPosArg(a))
		if params == nil {
			continue
		}
		i, ok := index[a.Class]
		if !ok {
			i = len(classes)
			index[a.Class] = i
			classes = append(classes, class{name: a.Class})
		}
		// the parameters that stand for arguments the AST keeps (positions, the thread and
		// the arena are dropped by the builder)
		for k, g := range a.Args {
			if isPositionArg(g) || k >= len(params) || params[k] == "" {
				continue
			}
			if !contains(classes[i].params, params[k]) {
				classes[i].params = append(classes[i].params, params[k])
			}
		}
	}
	var b strings.Builder
	fmt.Fprintf(&b, "// Code generated by parsegen from MySQL %s. DO NOT EDIT.\n\npackage %s\n\n", version, pkg)
	b.WriteString("// The typed views: one per parse-tree class the server builds, an accessor per\n// constructor parameter. Class names and parameter names are the server's.\n\n")
	for _, c := range classes {
		g := goName(c.name)
		fmt.Fprintf(&b, "// %s views a %s node.\ntype %s struct{ N *Node }\n\n", g, c.name, g)
		fmt.Fprintf(&b, "// As%s returns the view when v is a %s.\nfunc As%s(v Value) (%s, bool) {\n\tn, ok := v.(*Node)\n\tif !ok || n.Class != %s {\n\t\treturn %s{}, false\n\t}\n\treturn %s{n}, true\n}\n\n",
			g, c.name, g, g, strconv.Quote(c.name), g, g)
		...
		for _, p := range c.params {
			...
			seen[m] = true
			fmt.Fprintf(&b, "func (x %s) %s() Value { return x.N.Arg(%s) }\n", g, m, strconv.Quote(p))
		}
		b.WriteString("\n")
	}
	return b.String()
...
func isPositionArg(g Arg) bool {
	...
	return strings.HasPrefix(g.Text, "@") && len(g.Text) > 1 && g.Text[1] >= '0' && g.Text[1] <= '9'
}
...
// goName turns PT_query_specification into PTQuerySpecification and opt_where_clause into
// OptWhereClause.
