...
func (a *analyzer) predFacts(sc *scope, fs *facts.Scope, c mysqlast.Value, restrict []int) {
	...
	pr := facts.Pred{Op: facts.Opaque, Text: a.textOf(n), Origin: facts.FromStatement}
	if restrict != nil {
...
// writeFacts is the record of a write: its target and the columns it assigns.
func (a *analyzer) writeFacts(kind facts.StmtKind, rel *relation, assigned []*schema.Column, atMostOne bool) *facts.Facts {
	f := &facts.Facts{Kind: kind, AtMostOne: atMostOne}
