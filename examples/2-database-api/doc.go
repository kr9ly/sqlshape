// Package dbapi is the second sqlshape example: the database as an API. Tables are the
...
//   - Closed value sets: order_status (an enum) and customers.tier (CHECK (tier IN ...))
//     bind to Go types the same way — constants are diffed against the set and switches
//     must be exhaustive.
