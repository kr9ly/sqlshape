...
var loadItems = postgres.Copy[ItemIn]("order_items", "order_id", "line_no", "sku", "qty", "discount")
...
func TestAgainstPostgres(t *testing.T) {
	...
	note := "first"
	id1, err := postgres.First(ctx, db, insertOrder, NewOrder{UserID: 1, Total: "10.50", Note: &note})
	if err != nil {
		t.Fatalf("insert: %v", err)
	}
	id2, err := postgres.First(ctx, db, insertOrder, NewOrder{UserID: 2, Total: "99.00"})
	if err != nil {
	...
	if tag, err := postgres.Exec(ctx, db, markPaid, struct{ ID int64 }{id2}); err != nil || tag.RowsAffected() != 1 {
		t.Fatalf("exec: %v %v", tag, err)
	}

	all, err := postgres.Collect(ctx, db, listOrders, ListParams{})
	if err != nil {
	...
	rows, err := postgres.Collect(ctx, db, listOrders, ListParams{Status: &paid, Sort: "total"})
	if err != nil || len(rows) != 1 || rows[0].ID != id2 {
		t.Errorf("enum scalar param: %v %+v", err, rows)
	}
	rows, err = postgres.Collect(ctx, db, listOrders, ListParams{Statuses: []OrderStatus{"paid", "shipped"}})
	if err != nil || len(rows) != 1 {
		t.Errorf("enum array param: %v %+v", err, rows)
	}
	rows, err = postgres.Collect(ctx, db, listOrders, ListParams{IDs: []int64{id1}, Limit: 5})
	if err != nil || len(rows) != 1 || rows[0].ID != id2 {
		t.Errorf("range + with: %v %+v", err, rows)
	}

	_, err = postgres.First(ctx, db, insertOrder, NewOrder{UserID: 999, Total: "1"})
	if !postgres.Violates(err, "orders_user_id_fkey") {
		t.Errorf("fk violation not mapped: %v", err)
	}
	_, err = postgres.First(ctx, db, insertOrder, NewOrder{UserID: 1, Total: "1", Note: &note})
	var ce *postgres.ConstraintError
	...
	_, err = postgres.Exec(ctx, db, insertItemNoDiscount, struct{ OrderID int64 }{id1})
	var ceNN *postgres.ConstraintError
	...
	_, err = postgres.First(ctx, db, insertOrder, NewOrder{UserID: 1, Total: "-1"})
	if !postgres.Violates(err, "orders_total_check") {
		t.Errorf("check violation not mapped: %v", err)
	}
	_, err = postgres.First(ctx, db, insertOrder, NewOrder{UserID: 1, Total: "2000000"})
	if !postgres.Violates(err, "P0401") {
		t.Errorf("trigger SQLSTATE not mapped: %v", err)
	}

	one, err := postgres.Get(ctx, db, orderByID, struct{ ID int64 }{id2})
	if err != nil || one.ID != id2 || one.Status != "paid" {
		t.Errorf("One.Get: %v %+v", err, one)
	}
	if _, err := postgres.Get(ctx, db, orderByID, struct{ ID int64 }{id2 + 100}); !postgres.IsNoRows(err) {
		t.Errorf("One.Get missing: %v", err)
	}
	if _, ok, err := postgres.Find(ctx, db, orderByID, struct{ ID int64 }{id2 + 100}); err != nil || ok {
	...
	if row, err := postgres.Get(ctx, db, orderByID.Unprepared(), struct{ ID int64 }{id2}); err != nil || row.ID != id2 {
	...
	if po, err := postgres.Get(ctx, db, partialOrder, struct{ ID int64 }{id2}); err != nil || po.Note != nil {
		t.Errorf("partial result (optional field unset): %v %+v", err, po)
	}
	// ...and an error when it is not.
	if _, err := postgres.Collect(ctx, db, badPartialOrder, struct{ ID int64 }{id2}); err == nil || !strings.Contains(err.Error(), "no result column") {
	...
	if n, err := postgres.First(ctx, db, saveOrder, struct {
		Price MoneyIn
		Items []ItemIn
	}{MoneyIn{"1.00", "JPY"}, items}); err != nil || n == nil || *n != 1 {
		t.Fatalf("composite params: %v %d", err, n)
	}
	if skus, err := postgres.Collect(ctx, db, itemSkus, struct{ Items []ItemIn }{items}); err != nil || len(skus) != 2 || skus[0] != "A" || skus[1] != "B" {
		t.Errorf("composite[] param round trip: %v %v", err, skus)
	}
	if _, err := postgres.Exec(ctx, db, setPrice, struct {
		ID    int64
		Price *MoneyIn
	}{id2, &MoneyIn{"7.25", "USD"}}); err != nil {
		t.Fatalf("composite param in UPDATE: %v", err)
	}
	if pr, err := postgres.Get(ctx, db, priceOf, struct{ ID int64 }{id2}); err != nil || pr.Price == nil || *pr.Price.Amount != "7.25" || *pr.Price.Currency != "USD" {
	...
	if n, err := postgres.First(ctx, db, itemCount, struct{ OrderID int64 }{id1}); err != nil || n != 2 {
		t.Errorf("copied rows: %v %d", err, n)
	}

	// One.Exec: exactly one row, or ErrNoRows
	if _, err := postgres.ExecOne(ctx, db, markPaidOne, struct{ ID int64 }{id1}); err != nil {
		t.Errorf("One.Exec: %v", err)
	}
	if _, err := postgres.ExecOne(ctx, db, markPaidOne, struct{ ID int64 }{id1 + 100}); !postgres.IsNoRows(err) {
	...
	b := postgres.NewBatch()
	qAll := postgres.Queue(b, listOrders, ListParams{})
	qPaid := postgres.QueueOne(b, markPaidOne, struct{ ID int64 }{id1})
	qOne := postgres.QueueOne(b, orderByID, struct{ ID int64 }{id1})
	qNone := postgres.QueueOne(b, orderByID, struct{ ID int64 }{id1 + 100})
	if _, err := qAll.Rows(); !errors.Is(err, postgres.ErrNotSent) {
	...
	if _, err := qNone.First(); !postgres.IsNoRows(err) {
	...
	bLen := postgres.NewBatch()
	if bLen.Len() != 0 {
		t.Errorf("empty batch len: %d", bLen.Len())
	}
	postgres.Queue(bLen, listOrders, ListParams{})
	if bLen.Len() != 1 {
		t.Errorf("batch len after Queue: %d", bLen.Len())
	}
	postgres.QueueOne(bLen, priceTagOf, struct{ ID int64 }{id1})
	if bLen.Len() != 2 {
	...
	b = postgres.NewBatch()
	postgres.Queue(b, insertOrder, NewOrder{UserID: 999, Total: "1"})
	if err := b.Send(ctx, db); !postgres.Violates(err, "orders_user_id_fkey") {
	...
	if _, err := postgres.Collect(ctx, db, listOrders, ListParams{}); !errors.As(err, &ule) || ule.Value != "cancelled" {
	...
	if rows, err := postgres.Collect(ctx, db, listOrders.Unprepared(), ListParams{}); err != nil || len(rows) != 2 {
		t.Errorf("unprepared: %v %d", err, len(rows))
	}

	// materialized view refresh
	if err := postgres.MatView("order_stats").Refresh(ctx, db); err != nil {
		t.Errorf("refresh: %v", err)
	}
	if _, err := db.Exec(ctx, `CREATE UNIQUE INDEX order_stats_user_idx ON order_stats (user_id)`); err != nil {
		t.Fatal(err)
	}
	if err := postgres.MatView("order_stats").RefreshConcurrently(ctx, db); err != nil {
		t.Errorf("refresh concurrently: %v", err)
	}

	// procedures
	if _, err := postgres.Exec(ctx, db, markPaidProc, struct{ ID int64 }{id1}); err != nil {
		t.Errorf("CALL: %v", err)
	}
	if st, err := postgres.Get(ctx, db, settleProc, struct{ ID int64 }{id1}); err != nil || st.PTotal != "10.50" {
	...
	uao, err := postgres.Collect(ctx, db, userAnyOrders, struct{}{})
	...
	eo, err := postgres.First(ctx, db, embeddedRows, ByUser{1})
	...
	pr, err := postgres.Get(ctx, db, priceOf, struct{ ID int64 }{id1})
	if err != nil || pr.Price == nil || *pr.Price.Amount != "1.50" || *pr.Price.Currency != "JPY" {
		t.Errorf("composite column: %v %+v", err, pr.Price)
	}
	pr, err = postgres.Get(ctx, db, priceOf, struct{ ID int64 }{id2})
	...
	if _, err := postgres.Exec(ctx, db, setPriceTag, struct {
		ID    int64
		Price PriceTag
	}{id2, PriceTag{"(7.25,USD)"}}); err != nil {
		t.Fatalf("declared type param: %v", err)
	}
	if pt, err := postgres.Get(ctx, db, priceTagOf, struct{ ID int64 }{id2}); err != nil || pt.Price.Text != "(7.25,USD)" {
		t.Errorf("declared type result: %v %+v", err, pt.Price)
	}
	if pt, err := postgres.Get(ctx, db, priceTagOf, struct{ ID int64 }{id1 + 1000}); !postgres.IsNoRows(err) {
		t.Errorf("declared type missing row: %v %+v", err, pt)
	}
	if _, err := db.Exec(ctx, `UPDATE users SET handle = 'Alice', attrs = 'k=>v'`); err != nil {
		t.Fatal(err)
	}
	et, err := postgres.First(ctx, db, extTypes, struct{ H string }{"alice"})
	...
	if err := postgres.LoadUserTypes(ctx, db); err != nil {
	...
	sp, err := postgres.First(ctx, db, statusPairs, struct{}{})
	...
	if _, err := postgres.Exec(ctx, db, insertHost, h); err != nil {
		t.Fatalf("insert host: %v", err)
	}
	got, err := postgres.Get(ctx, db, hostByAddr, struct {
		Addr string
		Span pgtype.Range[int32]
	...
	first, err := postgres.First(ctx, db, listOrders, ListParams{IDs: []int64{id1, id2}})
	if !postgres.IsNoRows(err) {
		t.Errorf("First on empty: %v %+v", err, first)
	}

	n := 0
	for row, err := range postgres.Run(ctx, db, listOrders, ListParams{}) {
		if err != nil {
			t.Fatal(err)
		}
		n++
	...
	counts, err := postgres.Collect(ctx, db, countByStatus, struct{}{})
...
func TestConstraintErrorDirect(t *testing.T) {
	generic := &postgres.ConstraintError{Err: &pgconn.PgError{Message: "boom"}}
	...
	notNull := &postgres.ConstraintError{Table: "order_items", Column: "discount", Err: &pgconn.PgError{Message: "not null"}}
	if got := notNull.Key(); got != "order_items.discount" {
		t.Errorf("Key() (table.column) = %q", got)
	}
	if !postgres.Violates(notNull, "order_items.discount") {
...
func TestCopyLayoutErrors(t *testing.T) {
	...
	if _, err := postgres.Copy[any]("t").From(ctx, nil, nil); err == nil || !strings.Contains(err.Error(), "must not be an interface") {
		t.Errorf("interface R: %v", err)
	}
	if _, err := postgres.Copy[int64]("t", "a", "b").From(ctx, nil, nil); err == nil || !strings.Contains(err.Error(), "feeds exactly one column") {
		t.Errorf("scalar R, wrong column count: %v", err)
	}
	if _, err := postgres.Copy[ItemIn]("order_items", "nope").From(ctx, nil, nil); err == nil || !strings.Contains(err.Error(), "has no field") {
...
// noQueryDB implements postgres.BatchDB (SendBatch) but not postgres.DB: a batch
...
func TestRuntimeGaps(t *testing.T) {
	...
	id1, err := postgres.First(ctx, db, insertOrder, NewOrder{UserID: 1, Total: "10.00"})
	if err != nil {
		t.Fatalf("insert order 1: %v", err)
	}
	id2, err := postgres.First(ctx, db, insertOrder, NewOrder{UserID: 1, Total: "20.00"})
	if err != nil {
	...
	if _, err := postgres.First(ctx, db, anyStmt, struct{}{}); err == nil || !strings.Contains(err.Error(), "must not be an interface") {
		t.Errorf("interface R: %v", err)
	}

	// scalar R, wrong result column count.
	twoColStmt := sqlshape.Query[string, struct{}]("SELECT 'a', 'b'")
	if _, err := postgres.First(ctx, db, twoColStmt, struct{}{}); err == nil || !strings.Contains(err.Error(), "returns 2 columns") {
	...
	if tag, err := postgres.Get(ctx, db, scalarTagStmt, struct{}{}); err != nil || tag != "hello" {
	...
	if rows, err := postgres.Collect(ctx, db, lowerFallback, struct{}{}); err != nil || len(rows) == 0 {
	...
	if _, err := postgres.Collect(ctx, db, extraCol, struct{}{}); err == nil || !strings.Contains(err.Error(), "has no field") {
	...
	if _, err := postgres.Collect(ctx, db, dupCol, struct{}{}); err == nil || !strings.Contains(err.Error(), "matches") {
	...
	if _, err := postgres.Collect(ctx, db, dupTag, struct{}{}); err == nil || !strings.Contains(err.Error(), "both bind to column") {
	...
	if rows, err := postgres.Collect(ctx, db, embedVariants, struct{}{}); err != nil || len(rows) != 1 || rows[0].ID != 1 || rows[0].Extra != "x" {
	...
	if row, err := postgres.First(ctx, db, ptrEmbedStmt, struct{ ID int64 }{id1}); err != nil || row.PtrBase == nil || row.ID != id1 {
...
SELECT u.id, array_agg(row(o.id, o.total) ORDER BY o.id) AS orders FROM users u JOIN orders o ON o.user_id = u.id GROUP BY u.id ORDER BY u.id`)
	if rows, err := postgres.Collect(ctx, db, userOrdersPtr, struct{}{}); err != nil || len(rows) != 1 || len(rows[0].Orders) != 2 || rows[0].Orders[0] == nil {
...
SELECT u.id, array_agg(row(o.id, o.total) ORDER BY o.id) AS orders FROM users u JOIN orders o ON o.user_id = u.id GROUP BY u.id ORDER BY u.id`)
	if rows, err := postgres.Collect(ctx, db, oneFieldStmt, struct{}{}); err != nil || len(rows) != 1 || len(rows[0].Orders) != 2 || rows[0].Orders[0].ID == 0 {
...
SELECT u.id, array_agg(o ORDER BY o.id) FILTER (WHERE false) AS orders FROM users u JOIN orders o ON o.user_id = u.id GROUP BY u.id`)
	if rows, err := postgres.Collect(ctx, db, nullArrayStmt, struct{}{}); err != nil || len(rows) != 1 || rows[0].Orders != nil {
	...
	_, err = postgres.Exec(ctx, db, insertItemNoDiscount, struct{ OrderID int64 }{id1})
	if !postgres.Violates(err, "order_items.discount") {
...
SELECT o.id, o.status, o.total, o.note, o.created_at FROM orders o WHERE o.user_id = {{.UserID}}`)
	if _, _, err := postgres.Find(ctx, db, ordersByUser, struct{ UserID int64 }{1}); !errors.Is(err, postgres.ErrManyRows) {
		t.Errorf("Single.Find ErrManyRows: %v", err)
	}
	if _, err := postgres.Get(ctx, db, ordersByUser, struct{ UserID int64 }{1}); !errors.Is(err, postgres.ErrManyRows) {
	...
	if _, err := postgres.ExecOne(ctx, db, markPaidByUser, struct{ UserID int64 }{1}); !errors.Is(err, postgres.ErrManyRows) {
...
SELECT o.id, o.status, o.total, o.note, o.created_at FROM orders o WHERE o.id = {{.ID}}`)
	var ule *sqlshape.UnknownLabelError
	if _, _, err := postgres.Find(ctx, db, orderByIDErr, struct{ ID int64 }{id1}); !errors.As(err, &ule) {
	...
	if rows, err := postgres.Collect(ctx, db, optStatus, struct{}{}); err != nil || len(rows) != 1 || rows[0].Status != nil {
	...
	if _, err := postgres.First(ctx, db, statusPairs, struct{}{}); !errors.As(err, &ule) {
	...
	if _, err := postgres.Collect(ctx, db, badField, struct{}{}); err == nil || !strings.Contains(err.Error(), "has no field") {
		t.Errorf("Run Render error: %v", err)
	}
	if _, err := postgres.Exec(ctx, db, badField, struct{}{}); err == nil || !strings.Contains(err.Error(), "has no field") {
		t.Errorf("Exec Render error: %v", err)
	}
	badSQL := sqlshape.Query[struct{}, struct{}]("SELECT no_such_column_xyz FROM orders LIMIT 1")
	var pgErr *pgconn.PgError
	if _, err := postgres.Collect(ctx, db, badSQL, struct{}{}); !errors.As(err, &pgErr) || strings.HasPrefix(pgErr.Code, "23") {
	...
	cancelNow()
	if _, err := postgres.Exec(cancelledCtx, db, markPaid, struct{ ID int64 }{id1}); !errors.Is(err, context.Canceled) {
	...
	if _, err := postgres.First(ctx, nc, saveOrder, struct {
		Price MoneyIn
		Items []ItemIn
	...
	loadItemsAllCols := postgres.Copy[ItemIn]("order_items")
	...
	loadNames := postgres.Copy[string]("users", "name")
	...
	b := postgres.NewBatch()
	qBad := postgres.Queue(b, badField, struct{}{})
	if _, err := qBad.Tag(); err == nil {
		t.Errorf("Tag before send should be the render error, got nil")
	}
	qOK := postgres.Queue(b, listOrders, ListParams{})
	if _, err := qOK.Tag(); !errors.Is(err, postgres.ErrNotSent) {
	...
	if q := postgres.Queue(b, listOrders, ListParams{}); q == nil {
	...
	b2 := postgres.NewBatch()
	qScanner := postgres.QueueOne(b2, priceTagOf, struct{ ID int64 }{id1})
	...
	b2b := postgres.NewBatch()
	qMismatch := postgres.Queue(b2b, badPartialOrder, struct{ ID int64 }{id1})
	...
	b3 := postgres.NewBatch()
	postgres.Queue(b3, anyStmt, struct{}{})
	...
	b3b := postgres.NewBatch()
	postgres.Queue(b3b, dupTag, struct{}{})
	...
	// sql.Scanner.
	postgres.Queue(postgres.NewBatch(), sqlshape.Query[PriceTag, struct{}]("SELECT 1"), struct{}{})
	...
	b3c := postgres.NewBatch()
	qScanErr := postgres.Queue(b3c, listOrders, ListParams{})
	...
	nq := noQueryDB{c: db}
	b4 := postgres.NewBatch()
	postgres.QueueOne(b4, priceTagOf, struct{ ID int64 }{id1})
