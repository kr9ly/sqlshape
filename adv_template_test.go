...
func TestAdvTemplateSpliceViaIfElseBreaksAtRuntime(t *testing.T) {
	...
	_, err = advSpliceViaIfElse.Collect(ctx, db, advSpliceParams{A: false, Q: "SECRET"})
	if err == nil {
...
func TestAdvTemplateBareOrderBySecondColumnIsIgnored(t *testing.T) {
	...
	ids, err := advBareOrderBySecondColumn.Collect(ctx, db, advOrderByParams{Sort: "does-not-exist-as-a-column"})
	if err != nil {
