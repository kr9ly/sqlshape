...
func Queue[R, P any](b *Batch, s sqlshape.Stmt[R, P], p P) *Queued[R] {
	q := &Queued[R]{}
	if b.sent {
	...
	if hasUserScanner(reflect.TypeOf(zero)) {
		b.after = append(b.after, func(ctx context.Context, db BatchDB) error {
			q.done = true
			if d, ok := db.(DB); ok {
				q.rows, q.err = Collect(ctx, d, s, p)
			} else {
	...
	qq := b.b.Queue(r.SQL, pgArgs(r.Args)...)
...
func QueueOne[R, P any](b *Batch, s sqlshape.Single[R, P], p P) *Queued[R] {
...
func hasUserScanner(rt reflect.Type) bool {
	...
	if rt.Kind() != reflect.Struct || sqlshape.ScalarStruct(rt) {
