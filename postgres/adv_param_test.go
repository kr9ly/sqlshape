...
func TestAdvParamArrayOverflow(t *testing.T) {
	...
	_, err = postgres.Exec(ctx, db, insertTags, Params{ID: 1, Tags: []int64{40000}})
	if err == nil {
...
func TestAdvParamArrayPrecisionLoss(t *testing.T) {
	...
	_, err = postgres.Exec(ctx, db, insertReals, Params{ID: 1, Reals: []float64{notSent}})
	if err != nil {
	...
	got, err := postgres.First(ctx, db, readReals, struct{ ID int64 }{1})
	if err != nil {
